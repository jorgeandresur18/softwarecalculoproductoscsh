...
export default function SymptomFilters({ filters, onChange, total, filtered }: Props) {
  function set(partial: Partial<FilterState>) {
    onChange({ ...filters, ...partial });
  }

  function clear() {
    onChange({ search: '', status: 'active' });
  }

  const isDirty = filters.search !== '' || filters.status !== 'active';
